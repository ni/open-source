...
Describe 'BuildLvlibpDockerLinux.Workflow' {
    $meta = @{
        requirement = 'REQ-039'
        Owner       = 'NI'
        Evidence    = 'tests/pester/BuildLvlibpDockerLinux.Workflow.Tests.ps1'
    }
    ...
    It 'validates build-lvlibp-docker-linux action exists in dispatcher registry [REQ-039]' -Tag 'REQ-039' {
        $dispatchersPath = Join-Path $repoRoot 'dispatchers.json'
        $dispatchers = Get-Content $dispatchersPath | ConvertFrom-Json
        
        $dispatchers.'build-lvlibp-docker-linux' | Should -Not -BeNullOrEmpty
        $dispatchers.'build-lvlibp-docker-linux'.function | Should -Be 'Invoke-BuildLvlibpDockerLinux'
    }
    ...
    It 'validates required parameters are defined [REQ-039]' -Tag 'REQ-039' {
        ...
        $action = $dispatchers.'build-lvlibp-docker-linux'
    ...
    It 'validates module function throws when required parameters are missing [REQ-039]' -Tag 'REQ-039' {
        { Invoke-BuildLvlibpDockerLinux } | Should -Throw
    }
    ...
    It 'validates action.yml exists [REQ-039]' -Tag 'REQ-039' {
        $actionPath = Join-Path $repoRoot 'build-lvlibp-docker-linux' 'action.yml'
        Test-Path $actionPath | Should -Be $true
    }

    It 'validates implementation script exists [REQ-039]' -Tag 'REQ-039' {
        $scriptPath = Join-Path $repoRoot 'scripts' 'build-lvlibp-docker-linux' 'BuildLvlibpDockerLinux.ps1'
