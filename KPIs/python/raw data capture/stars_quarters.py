...
def lumps_for_repo(repo_df):
    ...
    lumps_info => { 'Q01': (start_dt, starVal or NaN), ...}
    """
    if repo_df.empty:
        return {}
    ...
    for i in range(1, MAX_LUMPS+1):
        ...
        if sub.empty:
            ...
            star_val=sub["monthly_count"].sum()
        lumps_info[label]=(lumps_start, star_val)
    return lumps_info
...
def main():
    ...
    all_s= df_s["repo_name"].unique().tolist()
    all_f= df_f["repo_name"].unique().tolist()
    all_repos= sorted(list(set(all_s+all_f)))
    ...
    # read star data => lumps => Q01..Q04 => pivot => scale => plots
    star_all = pd.read_sql("""
      SELECT
        repo_name,
    ...
    """,engine)
    star_all = star_all[ star_all["repo_name"].isin(enabled_repos) ].copy()
    star_all["dt"]= star_all.apply(build_date_for_row, axis=1)

    lumps_dict_list=[]
    lumps_start_dict_list=[]
    earliest_list=[]
    for repo, subdf in star_all.groupby("repo_name"):
    ...
    # check if SCALING_REPO is in lumps_pivot => col, and "Q01" in index => we can scale
    if SCALING_REPO in lumps_pivot_scaled.columns and "Q01" in lumps_pivot_scaled.index:
        ...
        if pd.isna(scale_ref_val) or scale_ref_val<=0:
            ...
            # scale_ref_val>0 => define scale => scale= scale_ref_val / thatRepoQ01 => actually we want
            # the lumps Q01 for that repo to remain the same => so the "lowest Q01" we do is scale_ref_val
            # meaning we want Q01 of that repo to remain "scale_ref_val" => so for each other repo => 
            # scale= scale_ref_val / thatOtherRepoQ01 => so that other Q01 => scale_ref_val
            for repo in lumps_pivot_scaled.columns:
            ...
            for repo in lumps_pivot_scaled.columns:
                ...
                # lumps table => line break + many decimals
                new_cols_table.append(f"{repo}\n(sf={sf:.8f})")
                # bar => single line => maybe 2 decimals
    ...
    lumps_start_disp= pd.DataFrame(lumps_start_dict_list)[lumps_start_cols].copy()
    ...
    ax_chart.set_title("Star growth over time")
    ax_chart.set_ylabel("Scaled Star Count")
    ax_chart.set_xlabel("")
    ...
    ############################################################
    # lumps_bar2 => scaled lumps => row= Qxx, col= "repo(sf=xx.xx)"
    # we need to identify which column in lumps_bar2 corresponds to SCALING_REPO
    # so let's find it by matching partial => e.g. starts with SCALING_REPO or so
    scaling_col = None
    ...
    # compute average ignoring the scaling repo
    lumps_avg = lumps_bar2.drop(columns=[scaling_col], errors="ignore").mean(axis=1)

    # We'll produce a side-by-side bar chart with 2 columns:
    ...
    if scaling_col in lumps_bar2.columns:
        ...
        # if we didn't find it or that repo not scaled => just skip
    ...
    ax2.set_ylabel("Scaled Star Count")
    ax2.set_xlabel("")
