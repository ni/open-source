...
def robust_get_page(session, url, params, handle_rate_limit_func, max_retries=20, endpoint="code_scanning"):
        ...
        while local_attempt<=mini_retry_attempts:
            try:
                ...
                if resp.status_code==200:
                    ...
                    logging.info("[deadbird/%s-etag] 304 => no new code scanning alerts" % endpoint)
                    return (resp,False)
                elif resp.status_code==403:
                    # fallback => insufficient perms => skip
                    logging.warning("[deadbird/%s] 403 => skipping code scanning => no perms" % endpoint)
                    return (resp,False)
                ...
                logging.warning("[deadbird/%s-etag] Connection error => mini => %s",endpoint,url)
                time.sleep(3)
                local_attempt+=1
        if local_attempt>mini_retry_attempts:
            logging.warning("[deadbird/%s-etag] exhausted => break => %s",endpoint,url)
            return (None,False)
    logging.warning("[deadbird/%s-etag] Exceeded => give up => %s",endpoint,url)
    return (None,False)
