...
def get_pulls_for_repo(conn, owner, repo):
    """
    Return a list of (owner, repo, pull_number) from 'pulls' for the given repo_name.
    Possibly filter by open or recently updated.
    """
    c = conn.cursor()
...
def get_pull_last_id(conn, repo_name, pull_number):
    """
    Return last_event_id from 'pulls' table.
    """
    c = conn.cursor()
    c.execute("""
