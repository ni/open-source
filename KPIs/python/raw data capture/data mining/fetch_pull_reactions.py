        ...
        while local_attempt<=mini_retry_attempts:
            try:
                ...
                if resp.status_code==200:
                    ...
                    logging.warning("[deadbird/pull_reactions] HTTP %d => attempt %d/%d => retry => %s",
                                    resp.status_code,attempt,max_retries,url)
                    time.sleep(5)
                else:
                    logging.warning("[deadbird/pull_reactions] HTTP %d => break => %s",
                                    resp.status_code,url)
                    return (resp,False)
                break
            except ConnectionError:
                logging.warning("[deadbird/pull_reactions] Connection error => local mini => %s",url)
                time.sleep(3)
                local_attempt+=1
        if local_attempt>mini_retry_attempts:
            logging.warning("[deadbird/pull_reactions] Exhausted => break => %s",url)
            return (None,False)
    logging.warning("[deadbird/pull_reactions] Exceeded => give up => %s",url)
    return (None,False)
