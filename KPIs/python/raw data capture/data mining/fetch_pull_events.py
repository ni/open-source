...
def fetch_pull_events(owner, repo, pull_number,
                      session, conn,
                      last_event_id=0,
                      overlap_pages=1,
    ...
    while True:
        ...
        if resp.status_code != 200:
            logging.warning("Pull events => HTTP %d => stopping %s/%s#%d",
        ...
        for evt in data:
            ...
            if etype in ("labeled","unlabeled"):
                ...
                if evt.get("commit_id"):
                    etype = "merged"

            insert_sql = """
            INSERT INTO pull_events
              (repo_name, pull_number, event_id, event_type, actor_login, created_at,
