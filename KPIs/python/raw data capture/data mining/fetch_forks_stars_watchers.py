...
We do NOT break on 403 => rotate tokens / sleep => robust_get_page => eventually success.

We pass handle_rate_limit_func, max_retries from main.py,
so no NameError: 'max_retries' is not defined.
"""
...
def robust_get_page(session, url, params, handle_rate_limit_func, max_retries=20):
    """
    Attempt a GET call up to max_retries times,
    calling handle_rate_limit_func(resp) each time to rotate tokens or sleep.
    Returns (resp, success_boolean).
    """
...
def list_watchers_single_thread(conn, owner, repo, enabled,
    ...
    """
    For watchers => no created_at => if enabled=1 => robust approach => no break on first 403
    """
    if enabled == 0:
...
def list_forks_single_thread(conn, owner, repo, baseline_date, enabled,
                             session, handle_rate_limit_func, max_retries):
    """
    For forks => skip if created_at>baseline_date => robust approach => no break on 403
    """
    if enabled == 0:
    ...
    while True:
        ...
        if not success:
            logging.warning("Forks => can't get page %d => break => %s/%s",
                            page, owner, repo)
            break
...
def list_stars_single_thread(conn, owner, repo, baseline_date, enabled,
                             session, handle_rate_limit_func, max_retries):
    """
    For stars => skip if starred_at>baseline_date => robust approach => no break on 403
    """
    if enabled == 0:
    ...
    while True:
        ...
        if new_base != baseline_date:
            baseline_date = new_base
            logging.info("Repo %s/%s => baseline changed => now %s (stars).",
