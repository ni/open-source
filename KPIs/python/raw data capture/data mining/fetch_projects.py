        ...
        while local_attempt <= mini_retry_attempts:
            try:
                ...
                if resp.status_code == 200:
                    ...
                    logging.warning(
                        "[deadbird/projects] HTTP %d => attempt %d/%d => retry => %s",
                        resp.status_code, attempt, max_retries, url
                    )
                    time.sleep(5)
                else:
                    logging.warning(
                        "[deadbird/projects] HTTP %d => attempt %d => break => %s",
...
def list_projects_single_thread(conn, owner, repo, enabled,
                                ...
                                max_retries):
    """
    Single-thread approach to fetch repository-level projects (/projects endpoint).
    main.py calls list_projects_single_thread from fetch_projects import ...
    """
    if enabled == 0:
    ...
    while True:
        ...
        # Must set Accept to the preview needed for Projects API
        ...
        new_count = 0
        for pj_obj in data:
            if store_project_and_columns(conn, repo_name, pj_obj, session, handle_rate_limit_func, max_retries):
                new_count += 1
        ...
        if last_page:
            ...
            if total_items > 0:
                logging.debug("[deadbird/projects] => total so far %d out of ~%d => %s",
...
def store_project_and_columns(conn, repo_name, pj_obj, session, handle_rate_limit_func, max_retries):
    """
    Insert/update the project record in repo_projects,
    then fetch columns via fetch_project_columns.
    Return True if we inserted a new row, False if updated.
    """
    c = conn.cursor()
...
def update_project_record(c, conn, repo_name, project_id, pj_obj):
    ...
    WHERE repo_name=%s AND project_id=%s
    """
    c.execute(sql, (name, body, state, created_dt, updated_dt, raw_str,
                    repo_name, project_id))
    conn.commit()
...
def fetch_project_columns(conn, repo_name, project_id, session, handle_rate_limit_func, max_retries):
    ...
    Then store/insert columns, then fetch cards inside each column.
    """
    ...
    while True:
        ...
        if not data:
            break
        for col_obj in data:
            store_project_column_and_cards(conn, repo_name, project_id, col_obj, session, handle_rate_limit_func, max_retries)
        if len(data) < 30:
            break
        page += 1
    session.headers["Accept"] = old_accept
