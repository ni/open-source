# Build Packed Library with Docker 🐳📦

Call **`BuildLvlibpDocker.ps1`** to compile LabVIEW packed libraries using LabVIEWCLI inside a Docker container.

## Inputs

| Name | Required | Example | Description |
|------|----------|---------|-------------|
| `minimum_supported_lv_version` | **Yes** | `2026` | LabVIEW version to use. |
...
| `image_tag` | No | `2026q1-linux` | Docker image tag. Defaults to `{version}q1-linux`. |

## Quick-start

The following example builds using LabVIEW 2026 inside a Docker container.

```yaml
- uses: ./.github/actions/build-lvlibp-docker
  with:
...
- The specified Docker image must contain LabVIEWCLI
- The LabVIEW project file must exist at the specified path

See also: [docs/actions/build-lvlibp-docker.md](../actions/build-lvlibp-docker.md)
