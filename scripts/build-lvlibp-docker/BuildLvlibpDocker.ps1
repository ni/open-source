...
.EXAMPLE
    .\BuildLvlibpDocker.ps1 -MinimumSupportedLVVersion "2026q1-linux" -SupportedBitness "64" -RelativePath "." -BuildSpec "Editor Packed Library" -Major 1 -Minor 0 -Patch 0 -Build 0 -Commit "abc1234" -DockerImage "nationalinstruments/labview"
...
try {
    ...
    $labviewPath = if ($IsWindows) {
        ...
        "/usr/local/natinst/LabVIEW-${MinimumSupportedLVVersion}-${SupportedBitness}/labview"
    }

    Write-Verbose "LabVIEWPath: $labviewPath"
    
    # Determine volume mount syntax based on OS
    $workspaceMount = if ($IsWindows) {
        "${PWD}:C:\workspace"
    } else {
        "${PWD}:/workspace"
    }

    $containerWorkDir = if ($IsWindows) {
        "C:\workspace"
    } else {
        "/workspace"
    }

    $containerProjectPath = if ($IsWindows) {
        "C:\workspace\$ProjectPath"
    } else {
    ...
    $cliArgs += "-Version `"$Major.$Minor.$Patch.$Build`""

    # Construct LabVIEWCLI command
    $labviewCLICommand = "LabVIEWCLI $($cliArgs -join ' ')"

    Write-Information "Executing LabVIEWCLI in Docker container..." -InformationAction Continue
    Write-Verbose "Command: $labviewCLICommand"
    ...
    $buildOutputs = Get-ChildItem -Path . -Filter "*.lvlibp" -Recurse | Where-Object { $_.Directory.Name -eq 'builds' }
